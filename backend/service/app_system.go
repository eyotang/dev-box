...
func (s *systemService) Start(ctx context.Context, version string) {
	...
	if screen, err := runtime.ScreenGetAll(ctx); err == nil && len(screen) > 0 {
		for _, sc := range screen {
			if sc.IsCurrent {
				if sc.Size.Width < consts.MinWindowWidth || sc.Size.Height < consts.MinWindowHeight {
...
func (s *systemService) Info() (resp types.JSRsp) {
...
func (s *systemService) SelectFile(title string, extensions []string) (resp types.JSRsp) {
	filters := container.Map(extensions, func(i int) runtime.FileFilter {
...
func (s *systemService) SaveFile(title string, defaultName string, extensions []string) (resp types.JSRsp) {
	filters := container.Map(extensions, func(i int) runtime.FileFilter {
		return runtime.FileFilter{
			Pattern: "*." + extensions[i],
		}
